...
export function TcleModalSecure({
    ...
    const [hasScrolledToBottom, setHasScrolledToBottom] = useState(isAlreadyUnlocked || viewerOnly);
    const [isAccepted, setIsAccepted] = useState(false);
    const [scrollProgress, setScrollProgress] = useState(isAlreadyUnlocked || viewerOnly ? 100 : 0);
    ...
    const handleOpenChange = (value: boolean) => {
        if (!value) {
            ...
            setHasScrolledToBottom(isAlreadyUnlocked || viewerOnly);
            setIsAccepted(false);
            setScrollProgress(isAlreadyUnlocked || viewerOnly ? 100 : 0);
            setHtmlContent("");
            setZoom(1.0);
        }
        onOpenChange(value);
    ...
    useEffect(() => {
        if (!open || isPdf || !documentUrl) return;
        ...
        <Dialog open={open} onOpenChange={handleOpenChange}>
            ...
            <DialogContent className="!fixed !inset-0 !w-screen !h-[100dvh] !max-w-none !translate-x-0 !translate-y-0 !rounded-none flex flex-col gap-0 p-0 overflow-hidden">
                    ...
                    <div className="p-4">
                        {!documentUrl ? (
                            <div className="flex items-center justify-center py-16 text-gray-400 gap-3">
                                ...
                                <span className="text-sm">Documento não encontrado na base de dados.</span>
                            </div>
                        ) : isPdf ? (
                            <PdfViewer
                                url={documentUrl}
                ...
                <div className="flex-shrink-0 bg-white border-t border-gray-100 shadow-[0_-4px_12px_rgba(0,0,0,0.06)] px-5 py-3 space-y-2">
                    ...
                    {!viewerOnly && !hasScrolledToBottom && (
                    ...
                    <div className="flex gap-2">
                        {viewerOnly ? (
                            <button
                                ...
                                className="w-full py-2.5 rounded-xl font-bold text-sm text-white bg-gray-800 hover:bg-gray-700 transition-colors"
                            >
                                Fechar Visualização
                            </button>
                        ) : (
